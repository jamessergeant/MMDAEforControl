...
class Model:
    ...
    def process(self,laser,goal,command):
        ...
        if len(inputs) == 3:
            self.y = self.model.function(inputs[0].data.astype(self.dtype),inputs[1].data.astype(self.dtype),inputs[2].data.astype(self.dtype))
        elif len(inputs) == 2:
            self.y = self.model.function(inputs[0].data.astype(self.dtype),inputs[1].data.astype(self.dtype))
        elif len(inputs) == 1:
            self.y = self.model.function(inputs[0].data.astype(self.dtype))
